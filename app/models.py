...
class RoadmapEntry(db.Model):
    ...
    trait_slug          = db.Column(db.String(64), nullable=False)
    streak              = db.Column(db.Integer, default=0)
    last_completed_date = db.Column(db.Date, nullable=True)
    total_completions   = db.Column(db.Integer, default=0)
    is_checked_today    = db.Column(db.Boolean, default=False)  # flip false on uncheck, prevents XP re-award
 
    # Store last awarded amounts so uncheck can reverse them exactly
    last_xp_awarded     = db.Column(db.Integer, default=0)
    last_points_awarded = db.Column(db.Integer, default=0)
 
    __table_args__ = (
        db.UniqueConstraint('user_id', 'trait_slug', name='unique_user_trait'),
    )
 
    def checked_today(self) -> bool:
        return self.last_completed_date == date.today() and self.is_checked_today
